...
public class AKLevelMatchManager : MonoBehaviour
{
    ...
    [SerializeField] AkLevelSceneManager sceneManager;
    ...
    public void StartPhase1()
    {
        ...
        sandBoxDialogues.AKWinDialogue.dialogueFinished = false;
        sandBoxDialogues.AKLoseDialogue.dialogueFinished = false;
        roundDelay = 1.5f;
        fakeHand = false;
    ...
    IEnumerator MatchPhase1()
    {
        ...
        healthSystem.staminaHitDamage = 25f; //Mub loses with 5 stamina hits
        ...
        while (healthSystem.getPlayerHealth() > 0 && healthSystem.getOpHealth() > 0)
        {
            ...
            if (healthSystem.getOpHealth() <= 50f)
            {
        ...
        if (healthSystem.getOpHealth() <= 0)
        {
            //won match
            Debug.Log("You win");
            sandBoxDialogues.TriggerAKWinDialogueDialogue();
        ...
        else
        {
            //lost match
            Debug.Log("You Lost");
            sandBoxDialogues.TriggerAKLoseDialogueDialogue();
    ...
    IEnumerator WinMatchBreak()
    {
        ...
        while (!sandBoxDialogues.AKWinDialogue.dialogueFinished)
        {
            yield return null;
        }
    ...
    IEnumerator LoseMatchBreak()
    {
        ...
        while (!sandBoxDialogues.AKLoseDialogue.dialogueFinished)
        {
            yield return null;
        }
